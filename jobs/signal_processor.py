from django.db.models.signals import post_save, post_delete # single的子类
...
# 1、使用 decorator 来注册 信号处理器
@receiver(signal=post_save, sender=Resume, dispatch_uid="resume_post_save_dispatcher")  # 对Resume这个模型的'保存后（post_save）'监听
...
# 2、手工注册信号处理器   # 对Resume这个模型的'删除后（post_delete）'监听
