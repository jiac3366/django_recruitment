...
class Job(models.Model):
    ...
    # 若不想看到创建人 可以在admin管理它的属性
    # on_delete设置外键数据如何处理的逻辑 1.直接忽略 2.级联删除 3.将数据设置NULL
    created_date = models.DateTimeField(verbose_name="创建日期", auto_now_add=True)
    # default=datatime.now
    modified_date = models.DateTimeField(verbose_name="修改日期", auto_now=True)

    class Meta:
        db_table = u'job'
        verbose_name = u'公司岗位'
        verbose_name_plural = u'公司岗位'
...
class Resume(models.Model):
    ...
    class Meta:
        verbose_name = _('应聘者简历')
        verbose_name_plural = _('应聘者简历')
